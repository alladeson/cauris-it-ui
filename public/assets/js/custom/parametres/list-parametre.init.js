...
let listParam = {
        ...
        datatable = $(".datatable").DataTable({
                ...
                "ajax": {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        GlobalScript.ajxRqtErrHandler(xhr, "alertify", "la récupération des types de facture");
                ...
                columns: [{
                        ...
                        "render": function(data, type, row, meta) {
                            return `` +
                                ...
                                `<input type="checkbox" class="form-check-input" id="listParamcheck${data}">` +
                                `<label class="form-check-label" for="listParamcheck${data}"></label>` +
                                `</div>`;
                        }
                    },
                    { data: 'id' },
    ...
    saParams: function(title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                listParam.saSucces(oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                listParam.saError(notitle, notext);
        });
    },
    saRemoveParams: function(el, title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                listParam.removeItem(el, oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                listParam.saError(notitle, notext);
        });
    },
    submitFormData: function(event) {
        ...
        var data = listParam.dataFormat(form)
        ...
        GlobalScript.request((dataId ? URL_PUT_ITEM.replace("__id__", dataId) : URL_POST_ITEM), (dataId ? 'PUT' : 'POST'), data).then(function(data) {
            ...
            // listParam.saSucces("Succès !", "Enregistrement effectué avec succès.")
    ...
    showItem: function(el) {
        ...
        GlobalScript.request(URL_GET_ITEM.replace("__id__", id), 'GET', null).then(function(data) {
            ...
            console.log(data)
            var itemObj = data;
            listParam.setShowingTable(itemObj);
    ...
    editItem: function(el) {
        ...
        GlobalScript.request(URL_GET_ITEM.replace("__id__", id), 'GET', null).then(function(data) {
            ...
            listParam.setformData($("div.add-new-modal").find('form'), itemObj);
    ...
    setShowingTable: function(itemObj) {
        ...
        $showClasseTable.find('.td-ifu').text(itemObj.ifu);
        $showClasseTable.find('.td-rcm').text(itemObj.rcm ? itemObj.rcm : "-");
        $showClasseTable.find('.td-name').text(itemObj.name);
        $showClasseTable.find('.td-telephone').text(itemObj.telephone);
        $showClasseTable.find('.td-email').text(itemObj.email);
        $showClasseTable.find('.td-ville').text(itemObj.ville);
        $showClasseTable.find('.td-address').text(itemObj.address);
        $showClasseTable.find('.td-activation-date').text(GlobalScript.dateFormat(itemObj.activationDate));
        $showClasseTable.find('.td-serial-key').text(itemObj.serialKey);
        $showClasseTable.find('.td-nim').text(itemObj.nim);
        $showClasseTable.find('.td-type-system').text(itemObj.typeSystem);
        $showClasseTable.find('.td-expiration').text(GlobalScript.dateFormat(itemObj.expiration));
    },
};
$(document).ready(function() {
    listParam.listInitalizer();
    // // Edit record
    // datatable.on('click', '.edit-item', function(e) {
    //     e.preventDefault();
    //     formChange = false;
    //     listParam.editItem($(this));
    // });

    // // Delete a record
    // datatable.on('click', '.remove-item', function(e) {
    //     e.preventDefault();
    //     // listParam.removeItem($(this));
    //     listParam.saRemoveParams($(this), "Êtes-vous sûr de vouloir supprimer cette catégorie d'articles ?", "Cette opération est irréversible !", "Oui, supprimer !", "Non, annuler !", "Supprimée !", "Catégorie d'articles supprimée avec succès.", "annulée !", "Opération annulée, rien n'a changé.");
    // });

    //Show record
    datatable.on('click', '.show-item', function(e) {
        e.preventDefault();
        listParam.showItem($(this));
    });
