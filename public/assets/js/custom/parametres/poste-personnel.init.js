...
let postePersonnel = {
                ...
                columns: [{
                        ...
                        "render": function(data, type, row, meta) {
                            return `` +
                                ...
                                `<input type="checkbox" class="form-check-input" id="postePersonnelcheck${data}">` +
                                `<label class="form-check-label" for="postePersonnelcheck${data}"></label>` +
                                `</div>`;
                        }
                    },
                    { data: 'id' },
    ...
    saParams: function(title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                postePersonnel.saSucces(oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                postePersonnel.saError(notitle, notext);
        });
    },
    saRemoveParams: function(el, title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                postePersonnel.removeItem(el, oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                postePersonnel.saError(notitle, notext);
        });
    },
    submitFormData: function(event) {
        ...
        var data = postePersonnel.dataFormat(form)
        console.log(data)
        postePersonnel.request((data.id ? URL_PUT_ITEM.replace("__id__", data.id) : URL_POST_ITEM), (data.id ? 'PUT' : 'POST'), data).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            datatable.ajax.reload();
            postePersonnel.saSucces("Succès !", "Enregistrement effectué avec succès.")
            $("div.add-new-modal").modal('hide')
            form[0].reset()
        }).catch(function(err) {
            // Run this when promise was rejected via reject()
            console.log(err)
            postePersonnel.saError("Erreur !", "Une erreur s'est produite lors de l'enregistrement.")
        });
    },
    showItem: function(el) {
        ...
        //console.log(id);
        postePersonnel.request(URL_GET_ITEM.replace("__id__", id), 'GET', null).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            var itemObj = JSON.parse(data);
            postePersonnel.setShowingTable(itemObj);
            $(".show-item-modal").modal('show')

        }).catch(function(err) {
            // Run this when promise was rejected via reject()
            console.log(err)
            postePersonnel.saError("Erreur !", "Une erreur s'est produite lors de l'affichage.")
        });
    },
    editItem: function(el) {
        ...
        let id = el.data("item-id");
        //console.log(id);
        //var response = postePersonnel.request(URL_GET_ITEM.replace("__id__", id), 'GET', null);
        postePersonnel.request(URL_GET_ITEM.replace("__id__", id), 'GET', null).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            var itemObj = JSON.parse(data);
            $("div.add-new-modal").find('h5.modal-title').text('Modification');
            postePersonnel.setformData($("div.add-new-modal").find('form'), itemObj);
            $(".add-new-modal").modal('show');
        }).catch(function(err) {
            // Run this when promise was rejected via reject()
            console.log(err)
            postePersonnel.saError("Erreur !", "Une erreur s'est produite lors de la modification.")
        });
    },
    removeItem: function(el, oktitle, oktext) {
        ...
        let id = el.data("item-id");
        // console.log(id);
        postePersonnel.request(URL_DELETE_ITEM.replace("__id__", id), 'DELETE', null).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            postePersonnel.saSucces(oktitle, oktext);
            datatable.ajax.reload();
            ...
            // Run this when promise was rejected via reject()
            console.log(err)
            postePersonnel.saError("Erreur !", "Une erreur s'est produite lors de la suppression.")
        });
    },
...
$(document).ready(function() {
    postePersonnel.listInitalizer();
    // Edit record
    datatable.on('click', '.edit-item', function(e) {
        e.preventDefault();
        postePersonnel.editItem($(this));
    });

    // Delete a record
    datatable.on('click', '.remove-item', function(e) {
        e.preventDefault();
        // postePersonnel.removeItem($(this));
        postePersonnel.saRemoveParams($(this), "Êtes-vous sûr de vouloir supprimer ce type de chambre ?", "Cette opération est irréversible !", "Oui, supprimer !", "Non, annuller !", "Supprimé !", "Type de chambre supprimé avec succès.", "Annullée !", "Opération annullée, rien n'a changé.");
    });

    //Show record
    datatable.on('click', '.show-item', function(e) {
        e.preventDefault();
        postePersonnel.showItem($(this));
    });
});