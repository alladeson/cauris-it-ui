...
let userGroup = {
        ...
        datatable = $(".datatable").DataTable({
                ...
                "ajax": {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        GlobalScript.ajxRqtErrHandler(xhr, "alertify", "la récupération des groupes d'utilisateur");
                ...
                columns: [{
                        ...
                        "render": function(data, type, row, meta) {
                            return `` +
                                ...
                                `<input type="checkbox" class="form-check-input" id="userGroupcheck${data}">` +
                                `<label class="form-check-label" for="userGroupcheck${data}"></label>` +
                                `</div>`;
                        }
                    },
                    { data: 'id' },
    ...
    saParams: function(title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                ...
                userGroup.saError(notitle, notext);
        });
    },
    saRemoveParams: function(el, title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                userGroup.removeItem(el, oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                userGroup.saError(notitle, notext);
        });
    },
    submitFormData: function(event) {
        ...
        var data = userGroup.dataFormat(form)
    ...
    setformData: function(form, item) {
        if (form.length) {
            ...
            choices[0].setChoiceByValue(item.role)
        }
    },
    dataFormat: function(form) {
        if (form.length) {
            data = {
                ...
                'role': GlobalScript.checkBlank(form.find("#role").val()),
            };
    ...
    setShowingTable: function(itemObj) {
        ...
        $showClasseTable.find('.td-description').text(itemObj.description ? itemObj.description : "-");
        $showClasseTable.find('.td-role').text(itemObj.role ? itemObj.role : "-");
    },
...
$(document).ready(function() {
    userGroup.listInitalizer();
    userGroup.choicesJsInit();
    // Edit record
    datatable.on('click', '.edit-item', function(e) {
        e.preventDefault();
        formChange = false;
        userGroup.editItem($(this));
    });
    ...
    $("div.add-new-modal").find('form').on("click", ".submit-button", function(event) {
        event.preventDefault();
        if (!$("div.add-new-modal").find('form').find("#role").val()) {
            alertify.warning("Veuillez sélectionner un rôle svp !")
            return;
        }
        userGroup.submitFormData(event);
    })
