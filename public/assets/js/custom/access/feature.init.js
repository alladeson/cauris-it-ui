...
let feature = {
        ...
        datatable = $(".datatable").DataTable({
                ...
                "ajax": {
                    ...
                    error: function(xhr, status, error) {
                        ...
                        GlobalScript.ajxRqtErrHandler(xhr, "alertify", "la récupération des fonctionnalités");
                ...
                columns: [{
                        ...
                        "render": function(data, type, row, meta) {
                            return `` +
                                ...
                                `<input type="checkbox" class="form-check-input" id="featurecheck${data}">` +
                                `<label class="form-check-label" for="featurecheck${data}"></label>` +
                                `</div>`;
                        }
                    },
                    { data: 'id' },
    ...
    saParams: function(title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                ...
                feature.saError(notitle, notext);
        });
    },
    saRemoveParams: function(el, title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                feature.removeItem(el, oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                feature.saError(notitle, notext);
        });
    },
    submitFormData: function(event) {
        ...
        var data = feature.dataFormat(form)
    ...
    setShowingTable: function(itemObj) {
        ...
        $showClasseTable.find('.td-code').text(itemObj.code);
...
$(document).ready(function() {
    feature.listInitalizer();
    // // Edit record
    // datatable.on('click', '.edit-item', function(e) {
    //     e.preventDefault();
    // formChange = false;
    //     feature.editItem($(this));
    // });

    // // Delete a record
    // datatable.on('click', '.remove-item', function(e) {
    //     e.preventDefault();
    //     // feature.removeItem($(this));
    //     feature.saRemoveParams($(this), "Êtes-vous sûr de vouloir supprimer cette catégorie d'articles ?", "Cette opération est irréversible !", "Oui, supprimer !", "Non, annuller !", "Supprimée !", "Catégorie d'articles supprimée avec succès.", "Annullée !", "Opération annullée, rien n'a changé.");
    // });

    //Show record
    datatable.on('click', '.show-item', function(e) {
        e.preventDefault();
        feature.showItem($(this));
    });
