...
let grhPersonnel = {
                ...
                columns: [{
                        ...
                        "render": function(data, type, row, meta) {
                            return `` +
                                ...
                                `<input type="checkbox" class="form-check-input" id="grhPersonnelcheck${data}">` +
                                `<label class="form-check-label" for="grhPersonnelcheck${data}"></label>` +
                                `</div>`;
                        }
                    },
                    {
                        ...
                        "render": function(data, type, row, meta) {
                            return `` +
                                ...
                                `</div>` +
                                `</div>` +
                                `<a href="javascript:void(0);" class="text-body">${row.nom + ' ' + row.prenom}</a>`;
                        }
                    },
                    { "data": "adresse" },
                    { "data": "email" },
                    { "data": "telephone" },
                    {
                        ...
                        "render": function(data, type, row, meta) {
                            return `` +
                                ...
                                `<a href="javascript:void(0);" class="badge badge-soft-primary font-size-11">${data}</a>` +
                                `</div>`;
                        }
                    },
                    {
    ...
    saParams: function(title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                grhPersonnel.saSucces(oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                grhPersonnel.saError(notitle, notext);
        });
    },
    saRemoveParams: function(el, title, text, confirmButtonText, cancelButtonText, oktitle, oktext, notitle, notext) {
        Swal.fire({
            ...
            e.value ?
                grhPersonnel.removeItem(el, oktitle, oktext) :
                e.dismiss === Swal.DismissReason.cancel &&
                grhPersonnel.saError(notitle, notext);
        });
    },
    submitFormData: function(event) {
        ...
        var data = grhPersonnel.dataFormat(form)
        var posteId = form.find("#poste").val();
        console.log(data)
        grhPersonnel.request((data.id ? (URL_PUT_ITEM.replace("__id__", data.id)).replace("__idPoste__", posteId) : URL_POST_ITEM.replace("__idPoste__", posteId)), (data.id ? 'PUT' : 'POST'), data).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            datatable.ajax.reload();
            grhPersonnel.saSucces("Succès !", "Enregistrement effectué avec succès.")
            $("div.add-new-modal").modal('hide')
            form[0].reset()
        }).catch(function(err) {
            // Run this when promise was rejected via reject()
            console.log(err)
            grhPersonnel.saError("Erreur !", "Une erreur s'est produite lors de l'enregistrement.")
        });
    },
    showItem: function(el) {
        ...
        //console.log(id);
        grhPersonnel.request(URL_GET_ITEM.replace("__id__", id), 'GET', null).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            var itemObj = JSON.parse(data);
            grhPersonnel.setShowingTable(itemObj);
            $(".show-item-modal").modal('show')

        }).catch(function(err) {
            // Run this when promise was rejected via reject()
            console.log(err)
            grhPersonnel.saError("Erreur !", "Une erreur s'est produite lors de l'affichage.")
        });
    },
    editItem: function(el) {
        ...
        let id = el.data("item-id");
        //console.log(id);
        //var response = grhPersonnel.request(URL_GET_ITEM.replace("__id__", id), 'GET', null);
        grhPersonnel.request(URL_GET_ITEM.replace("__id__", id), 'GET', null).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            var itemObj = JSON.parse(data);
            $("div.add-new-modal").find('h5.modal-title').text('Modification');
            grhPersonnel.setformData($("div.add-new-modal").find('form'), itemObj);
            $(".add-new-modal").modal('show');
            grhPersonnel.getPostePersonnels(itemObj.poste.id);
        }).catch(function(err) {
            // Run this when promise was rejected via reject()
            console.log(err)
            grhPersonnel.saError("Erreur !", "Une erreur s'est produite lors de la modification.")
        });
    },
    removeItem: function(el, oktitle, oktext) {
        ...
        let id = el.data("item-id");
        // console.log(id);
        grhPersonnel.request(URL_DELETE_ITEM.replace("__id__", id), 'DELETE', null).then(function(data) {
            // Run this when your request was successful
            console.log(JSON.parse(data))
            grhPersonnel.saSucces(oktitle, oktext);
            datatable.ajax.reload();
            ...
            // Run this when promise was rejected via reject()
            console.log(err)
            grhPersonnel.saError("Erreur !", "Une erreur s'est produite lors de la suppression.")
        });
    },
    ...
    setformData: function(form, item) {
        ...
        grhPersonnel.setformDataOnStatutChange();
    },
    ...
    newItemEvent: function(event) {
        ...
        grhPersonnel.getPostePersonnels(null);
    },
    ...
    setShowingTable: function(itemObj) {
        ...
        $showClasseTable.find('.td-salaire').find('a').text(itemObj.salaire ? itemObj.salaire : 0);
    },
    getPostePersonnels: function(itemId) {
        grhPersonnel.request(URL_LIST_POSTE_PERSONNEL, 'GET', null).then(function(data) {
            // Run this when your request was successful
            ...
            // choices[0].setValue(postePersonnels);
...
$(document).ready(function() {
    grhPersonnel.listInitalizer();
    // Edit record
    datatable.on('click', '.edit-item', function(e) {
        e.preventDefault();
        grhPersonnel.editItem($(this));
    });

    // Delete a record
    datatable.on('click', '.remove-item', function(e) {
        e.preventDefault();
        // grhPersonnel.removeItem($(this));
        grhPersonnel.saRemoveParams($(this), "Êtes-vous sûr de vouloir supprimer ce personnel ?", "Cette opération est irréversible !", "Oui, supprimer !", "Non, annuller !", "Supprimé !", "Personnel supprimé avec succès.", "Annullée !", "Opération annullée, rien n'a changé.");
    });

    //Show record
    datatable.on('click', '.show-item', function(e) {
        e.preventDefault();
        grhPersonnel.showItem($(this));
    });
});
document.addEventListener("DOMContentLoaded", function() {
    grhPersonnel.choicesJsInit();
});